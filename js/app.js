(() => {
    ...
    function expandSpanText() {
        ...
        if (0 === spanElements.length) return;
    ...
    cardButton.onclick = () => {
        ...
        cardFon.onclick = EventTarget => {
            if ("bottom-header__cart--open" === EventTarget.target.className) cardButton.classList.remove("open");
        };
    };
    ...
    if (animItem.length > 0) {
        ...
        function animOnScroll() {
            animItem.forEach((elem => {
                ...
                if (pageYOffset > animItemOffset - animItemPoint && pageYOffset < animItemOffset + animItemHeight) animItem.classList.add("element-show"); else if (!animItem.hasAttribute("data-animAppear-NoHide")) animItem.classList.remove("element-show");
            }));
        }
        function offset(el) {
    ...
    window.addEventListener("scroll", (() => {
        ...
        for (let i = 0; i < visibilityArray.length; i++) if (visibilityArray[i].visible) {
